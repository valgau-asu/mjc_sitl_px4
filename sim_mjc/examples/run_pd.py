#!/usr/bin/env python3
"""Run the cascaded PD controller on both standard tasks.

Writes a scorecard, three diagnostic figures and a chase-camera video per task into
`out/pd_<task>/`.

    python3 run_pd.py                # both tasks, plots + video
    python3 run_pd.py --no-video     # much faster; rendering dominates
    python3 run_pd.py --show         # pop the figures up as well
    python3 run_pd.py --task hover   # just one
"""
import argparse
import sys
