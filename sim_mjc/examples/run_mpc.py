#!/usr/bin/env python3
"""Run the linear MPC controller on both standard tasks.

Writes a scorecard, three diagnostic figures and a chase-camera video per task into
`out/mpc_<task>/`.

    python3 run_mpc.py                # both tasks, plots + video
    python3 run_mpc.py --no-video     # much faster; rendering dominates
    python3 run_mpc.py --show         # pop the figures up as well
    python3 run_mpc.py --task hover   # just one
"""
import argparse
import sys
